...
interface Props {
  ...
  tourId: string;
  userId: string;
  // For stage-attached classifications the reveal pivot is the stage start_time,
  // not c.deadline (which is null). Server derives the same value for RLS/scoring.
...
export function ClassificationCard({
  ...
  const effectiveDeadline = deadlineOverride ?? c.deadline;
        ...
        >
          ...
          <span className="text-slate-200">
            {tips
              ...
              .map((t) =>
                ordered ? `${t.slot}. ${t.rider?.name}` : t.rider?.name,
              )
              .join(", ")}
          </span>
        </li>
