...
import { listClassifications, listMyClassificationTips, listRiders } from '../lib/queries'
...
export function Specials() {
  ...
  return (
    <div className="flex flex-col gap-3 py-3">
      <p className="text-sm text-slate-400">Sonderwertungen — tippbar bis zur jeweiligen Deadline.</p>
      {cls.map((c) => (
        <ClassificationCard
          key={c.id}
          classification={c}
      ...
      {cls.length === 0 && <p className="text-slate-400">Noch keine Wertungen.</p>}
    </div>
  )
}
