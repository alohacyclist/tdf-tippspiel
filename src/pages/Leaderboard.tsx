...
export function Leaderboard() {
  ...
  useEffect(() => {
    getLeaderboard(tour.id).then(setRows).catch((e) => setError(e.message))
  }, [tour.id])
        ...
        {rows.map((r, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="font-semibold text-yellow-400">{r.total_points} Pkt</span>
            </li>
          )
        })}
        {rows.length === 0 && <p className="text-slate-400">Noch keine Spieler.</p>}
      </ul>
    </div>
  )
}
