-- 0013_leaderboard_v4.sql
-- Final results rank ONLY by stage-winner points. Classification (Sonderwertungen)
-- and question points are exposed as separate info columns, not folded into the rank.
-- CTEs are copied verbatim from 0011; only the projected columns change, so the view
-- must be dropped + recreated (CREATE OR REPLACE cannot drop/reorder columns).
...
select
  ...
  coalesce(cp.points, 0) as special_points,
  coalesce(qp.points, 0) as question_points
from tours t
